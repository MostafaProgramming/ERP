...
CREATE FUNCTION public.update_inventory_on_purchase_order() RETURNS trigger
    ...
    AS $$
DECLARE
    loc_id INT; -- Variable to store the location_id
BEGIN
    -- Check if the status is being changed to 'Completed'
    IF NEW.status = 'Completed' AND OLD.status != 'Completed' THEN
        -- Retrieve the location_id from the purchase_orders table
        ...
        SET quantity = quantity + NEW.quantity,
            last_updated = NOW()
        WHERE product_id = NEW.product_id AND location_id = loc_id;

        -- If the product does not exist in the inventory, create a new record
        IF NOT FOUND THEN
            INSERT INTO inventory (product_id, location_id, quantity, last_updated)
            VALUES (NEW.product_id, loc_id, NEW.quantity, NOW());
        END IF;
    END IF;
    RETURN NEW;
END;
...
--
-- Name: update_inventory_on_stock_transfer(); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.update_inventory_on_stock_transfer() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    src_loc_id INT; -- Variable to store the source_location_id
    dest_loc_id INT; -- Variable to store the destination_location_id
BEGIN
    -- Check if the status is being changed to 'Completed'
    IF NEW.status = 'Completed' AND OLD.status != 'Completed' THEN
        -- Retrieve the source and destination location IDs from the stock_transfers table
        SELECT source_location_id, destination_location_id INTO src_loc_id, dest_loc_id
        ...
        SET quantity = quantity - NEW.quantity,
            last_updated = NOW()
        WHERE product_id = NEW.product_id AND location_id = src_loc_id;

        -- Check if the product exists in the inventory at the destination location
        IF EXISTS (
            SELECT 1
            FROM inventory
            WHERE product_id = NEW.product_id AND location_id = dest_loc_id
        ) THEN
        ...
        ELSE
            ...
            INSERT INTO inventory (product_id, location_id, quantity, last_updated)
            VALUES (NEW.product_id, dest_loc_id, NEW.quantity, NOW());
        END IF;
    END IF;
    RETURN NEW;
END;
...
ALTER FUNCTION public.update_inventory_on_stock_transfer() OWNER TO postgres;
...
--
-- Name: update_total_stock_on_purchase_order(); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.update_total_stock_on_purchase_order() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    -- Check if the status is being changed to 'Completed'
    IF NEW.status = 'Completed' AND OLD.status != 'Completed' THEN
        -- Update the total stock level in the product table
        UPDATE product
...
ALTER FUNCTION public.update_total_stock_on_purchase_order() OWNER TO postgres;
...
CREATE TABLE public.location (
    ...
    location_type character varying(25)
);
...
CREATE TABLE public.orders (
    ...
    order_type character varying(25),
    product_id integer,
...
1	4	17000.00	2025-01-21	2026-01-21	Supplier Research	Research
\.
...
41	1250.00	4	113	Utilities	2025-01-21	Expense for procurement utilities	15
\.
...
1	1001	1	135	2025-01-19
51	1003	\N	10	2025-01-19
50	1006	5	34	2025-01-19
52	1006	\N	12	2025-01-19
10	1002	5	31	2025-01-20
53	1002	\N	2	2025-01-20
54	1003	\N	10	2025-01-20
11	1003	1	40	2025-01-20
14	1003	4	80	2025-01-20
26	1009	1	38	2025-01-21
32	1010	2	55	2025-01-21
\.
...
COPY public.location (location_id, location_name, location, manager_id, contact_number, location_type) FROM stdin;
1	Las Vegas Store	Las Vegas, NV	1	702-555-0100	Store\n
2	Sydney Store	Sydney, Australia	2	61-2-5550-1234	Store
5	New York Warehouse	New York, NY	3	212-555-0200	Warehouse
3	New York Store	New York, NY	4	212-555-0200	Store
4	Washington Warehouse	Washington	5	212-555-0200	Warehouse
12	Test Store	Location Store Test	11	234234213	Store
13	Test Warehouse	Warehouse Location Test	9	38947543	Warehouse
\.
...
COPY public.orders (order_id, order_type, product_id, quantity, order_date, status, total_amount, created_at, updated_at, delivery_date) FROM stdin;
1001	purchase_order	1002	15	2024-01-15	Completed	3000.00	2025-01-16	2025-01-16	2024-02-05
1002	purchase_order	1004	8	2024-01-25	Completed	4000.00	2025-01-16	2025-01-16	2024-02-15
1004	purchase_order	1003	25	2024-01-20	Cancelled	7500.00	2025-01-16	2025-01-16	\N
1006	purchase_order	1005	6	2024-02-01	Completed	2000.00	2025-01-16	2025-01-16	2025-01-05
1009	purchase_order	1001	13	2025-01-08	Awaiting Approval	32500.00	2025-01-16	2025-01-16	\N
1011	purchase_order	1001	15	2025-01-19	Completed	13500.00	2025-01-19	2025-01-19	\N
1003	purchase_order	1006	12	2024-02-05	Completed	6000.00	2025-01-16	2025-01-16	\N
1005	purchase_order	1002	19	2024-01-10	Pending	5000.00	2025-01-16	2025-01-20	\N
1007	purchase_order	1002	2	2025-01-08	Completed	24000.00	2025-01-16	2025-01-20	2025-01-20
1010	stock_transfer	1003	10	2025-01-05	Completed	12000.00	2025-01-16	2025-01-20	2025-01-20
1008	purchase_order	1003	2	2025-01-08	Pending	3600.00	2025-01-16	2025-01-20	\N
1012	stock_transfer	1003	10	2025-01-20	Awaiting Approval	\N	2025-01-20	2025-01-20	\N
1013	purchase_order	1005	4	2025-01-20	Awaiting Approval	12800.00	2025-01-20	2025-01-20	\N
1014	purchase_order	1010	6	2025-01-20	Awaiting Approval	5700.00	2025-01-20	2025-01-20	\N
\.
...
53	Bob Test	bob.test@test.com	Store Employee	35000.00	5	2	2025-01-21	\N
...
1006	Thrust Reverser	Engine Component	15000.00	49	20	2024-02-05	6	12500.00
1002	Combustion Chamber	Engine Component	12000.00	136	50	2024-01-15	2	4000.00
11	Test Fan Blade	Testing	1400.00	23	15	2025-01-21	3	900.00
1009	Bearing Housing	Engine Component	3500.00	182	50	2024-02-20	3	2800.00
1010	Thermal Insulation Blanket	Engine Component	1500.00	323	90	2024-02-25	4	950.00
...
COPY public.purchase_orders (order_id, supplier_id, location_id) FROM stdin;
1001	2	4
1002	4	5
1003	6	5
1004	3	4
...
41	1	4	1009	2	2025-01-21	7000.00	Credit Card
1	1	4	1001	5	2025-01-05	12500.00	Credit Card
2	1	4	1003	10	2025-01-06	18000.00	Cash
3	1	4	1007	3	2025-01-07	13500.00	Debit Card
4	2	5	1002	2	2025-01-05	24000.00	Cash
5	2	5	1005	4	2025-01-06	20000.00	Credit Card
6	2	5	1008	6	2025-01-07	21000.00	Debit Card
7	3	11	1004	7	2025-01-05	5600.00	Cash
8	3	11	1006	1	2025-01-06	15000.00	Credit Card
9	3	11	1010	3	2025-01-07	4500.00	Debit Card
10	1	4	1001	10	2025-01-01	25000.00	Credit Card
11	1	4	1002	5	2025-01-02	60000.00	Cash
12	1	4	1003	8	2025-01-05	14400.00	Debit Card
13	1	4	1004	20	2025-01-10	16000.00	Credit Card
14	1	4	1005	3	2025-02-01	15000.00	Cash
15	1	4	1006	2	2025-02-15	30000.00	Credit Card
16	1	4	1007	5	2025-02-20	22500.00	Cash
17	1	4	1008	10	2025-03-01	35000.00	Credit Card
18	1	4	1009	4	2025-03-15	14000.00	Debit Card
19	1	4	1010	6	2025-03-20	9000.00	Cash
20	2	5	1001	12	2025-01-01	30000.00	Cash
21	2	5	1003	15	2025-01-12	27000.00	Debit Card
22	2	5	1004	25	2025-02-05	20000.00	Credit Card
23	2	5	1005	10	2025-02-14	50000.00	Cash
24	2	5	1006	4	2025-02-28	60000.00	Debit Card
25	2	5	1007	7	2025-03-10	31500.00	Cash
26	2	5	1008	6	2025-03-15	21000.00	Credit Card
27	2	5	1009	8	2025-03-20	28000.00	Cash
28	2	5	1010	10	2025-03-25	15000.00	Debit Card
29	2	5	1002	6	2025-03-30	72000.00	Credit Card
30	3	11	1001	5	2025-01-03	12500.00	Debit Card
31	3	11	1002	3	2025-01-15	36000.00	Cash
32	3	11	1003	7	2025-02-05	12600.00	Credit Card
33	3	11	1005	2	2025-02-12	10000.00	Cash
34	3	11	1007	6	2025-02-20	27000.00	Debit Card
35	3	11	1009	4	2025-02-25	14000.00	Credit Card
36	3	11	1010	3	2025-03-05	4500.00	Cash
37	3	11	1004	8	2025-03-10	6400.00	Debit Card
38	3	11	1008	2	2025-03-15	7000.00	Credit Card
39	3	11	1006	1	2025-03-20	15000.00	Cash
40	1	4	1006	5	2025-04-13	12500.00	Credit Card
42	2	5	1010	5	2024-12-18	7500.00	Debit Card
\.
...
COPY public.stock_transfers (order_id, source_location_id, destination_location_id) FROM stdin;
1012	1	1
1010	4	1
\.
...
COPY public.store (store_id, location_id, operating_hours, customer_capacity, parking_spaces) FROM stdin;
2	2	09:00-17:00	100	40
3	3	09:00-17:00	150	70
1	1	09:00-15:00	300	120
4	12	09:00-14:00	250	100
\.
...
7	Sample Supplier	123-456-7890	Sample Location	Sample Contract Terms
\.
...
COPY public.warehouse (warehouse_id, location_id, storage_capacity, number_of_docks) FROM stdin;
1	4	3240.00	5
2	5	3540.00	6
3	13	12345.00	4
\.
...
SELECT pg_catalog.setval('public.expenses_expense_id_seq', 41, true);
...
SELECT pg_catalog.setval('public.inventory_inventory_id_seq', 54, true);
...
SELECT pg_catalog.setval('public.orders_order_id_seq', 1014, true);
...
SELECT pg_catalog.setval('public.person_person_id_seq', 53, true);
...
SELECT pg_catalog.setval('public.salesrecord_sales_id_seq', 42, true);
...
ALTER TABLE ONLY public.stock_transfers
    ADD CONSTRAINT stock_transfers_pkey PRIMARY KEY (order_id);
...
--
-- Name: orders trigger_update_inventory_on_purchase; Type: TRIGGER; Schema: public; Owner: postgres
--

CREATE TRIGGER trigger_update_inventory_on_purchase AFTER UPDATE ON public.orders FOR EACH ROW WHEN (((old.status)::text IS DISTINCT FROM (new.status)::text)) EXECUTE FUNCTION public.update_inventory_on_purchase_order();
...
--
-- Name: orders trigger_update_inventory_on_stock_transfer; Type: TRIGGER; Schema: public; Owner: postgres
--

CREATE TRIGGER trigger_update_inventory_on_stock_transfer AFTER UPDATE ON public.orders FOR EACH ROW WHEN (((old.status)::text IS DISTINCT FROM (new.status)::text)) EXECUTE FUNCTION public.update_inventory_on_stock_transfer();
...
--
-- Name: orders trigger_update_total_stock_on_purchase_order; Type: TRIGGER; Schema: public; Owner: postgres
--

CREATE TRIGGER trigger_update_total_stock_on_purchase_order AFTER UPDATE ON public.orders FOR EACH ROW WHEN ((((old.status)::text IS DISTINCT FROM (new.status)::text) AND ((new.order_type)::text = 'purchase_order'::text))) EXECUTE FUNCTION public.update_total_stock_on_purchase_order();
...
ALTER TABLE ONLY public.location
    ADD CONSTRAINT fk_location_manager FOREIGN KEY (manager_id) REFERENCES public.person(person_id);
...
ALTER TABLE ONLY public.purchase_orders
    ADD CONSTRAINT purchase_orders_location_id_fkey FOREIGN KEY (location_id) REFERENCES public.location(location_id);


--
-- Name: purchase_orders purchase_orders_order_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.purchase_orders
    ADD CONSTRAINT purchase_orders_order_id_fkey FOREIGN KEY (order_id) REFERENCES public.orders(order_id);
...
ALTER TABLE ONLY public.sales_record
    ADD CONSTRAINT salesrecord_manager_id_fkey FOREIGN KEY (manager_id) REFERENCES public.person(person_id);
...
ALTER TABLE ONLY public.sales_record
    ADD CONSTRAINT salesrecord_store_id_fkey FOREIGN KEY (store_id) REFERENCES public.location(location_id);


--
-- Name: stock_transfers stock_transfers_destination_location_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.stock_transfers
    ADD CONSTRAINT stock_transfers_destination_location_id_fkey FOREIGN KEY (destination_location_id) REFERENCES public.location(location_id);


--
-- Name: stock_transfers stock_transfers_order_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.stock_transfers
    ADD CONSTRAINT stock_transfers_order_id_fkey FOREIGN KEY (order_id) REFERENCES public.orders(order_id);


--
-- Name: stock_transfers stock_transfers_source_location_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.stock_transfers
    ADD CONSTRAINT stock_transfers_source_location_id_fkey FOREIGN KEY (source_location_id) REFERENCES public.location(location_id);
